...
def map_data_to_form(data_row, template_ws, output_ws, start_row):
    ...
    for row_idx in range(1, form_height + 1):
        ...
        if label_cell.value:
            ...
            # Determine where to put the data (usually column B)
            target_data_cell = output_ws.cell(row=start_row + row_idx - 1, column=2)
            ...
            if 'nama bayi' in label_text or 'nama anak' in label_text:
                ...
                if 'NIK' in data_row.index:
                    ...
                    # Handle NIK formatting (remove scientific notation)
                    if pd.notna(nik_value):
                        if isinstance(nik_value, (int, float)):
                            target_data_cell.value = f"{int(nik_value)}"
                        else:
                            target_data_cell.value = str(nik_value)
            ...
            elif 'nama ayah' in label_text or ('ayah' in label_text and 'ibu' in label_text):
                ayah = data_row.get('AYAH', '') if 'AYAH' in data_row.index else ''
                ibu = data_row.get('IBU', '') if 'IBU' in data_row.index else ''
                
                if ayah and ibu:
                    target_data_cell.value = f"Ayah: {ayah}, Ibu: {ibu}"
                elif ayah:
                    target_data_cell.value = f"Ayah: {ayah}"
                elif ibu:
                    target_data_cell.value = f"Ibu: {ibu}"
            
            elif 'alamat' in label_text:
                if 'ALAMAT' in data_row.index:
                    target_data_cell.value = data_row['ALAMAT']
            
            elif 'no' in label_text and 'hp' in label_text:
                if 'No. Hp' in data_row.index:
                    target_data_cell.value = data_row['No. Hp']
